...
<div class="panel admin-panel margin-top">
  ...
  <div class="body-content">
    <form method="post" class="form-x" action="{{url('admin/cate_child')}}" id="add_cate">
    {!! csrf_field() !!}
      <div class="form-group">
        ...
        <div class="field">
          <select name="pid" id="pid" class="input w50">
            <option <?php echo $id==0? 'selected':'' ?> value="0">顶级分类</option>
            @if($types)
              @foreach($types as $val)
                <option <?php echo $id==$val['id']? 'selected':'' ?> value="{{$val['id']}}">{{$val['name']}}</option>
              @endforeach
            @endif
          </select>
      ...
      <div class="form-group">
        ...
        <div class="field">
          <input type="text" class="input w50" name="title" value="" id="title" />
          <div class="tips"></div>
        </div>
      </div>
